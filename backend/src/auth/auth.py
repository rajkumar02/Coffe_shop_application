...
AUTH0_DOMAIN = "rajkumar1.auth0.com"
ALGORITHMS = ["RS256"]
API_AUDIENCE = "coffes"
...
def get_token_auth_header():
    auth = request.headers.get("Authorizations", None)
    if not auth:
        raise AuthError({
    ...
    sep = auth.split()
    if sep[0].lower() != "bearer":
        raise AuthError({
            "code": "Header not valid",
            "description": "Authorization header 'Bearer' not found."
        ...
        raise AuthError({
            "code": "Header not valid",
            "description": " 'Bearer' not found in authorization header"
        }, 401)
